...
import { historyAction, statAction, userAction, wishlistAction } from "@/actions";
...
const Fetcher = async ({ address }: { address: string }): Promise<JSX.Element> => {
  ...
  let pendingNotifications: Record<string, boolean> = {};
  ...
  if (isOwner) {
    pendingNotifications = await historyAction.getUserHistoryPerAudit(address);
  }
